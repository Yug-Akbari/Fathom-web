...
interface FbmBatch { id: string; name: string; createdAt: any; }
...
function AccountingContent() {
  ...
  const [batches, setBatches] = useState<FbmBatch[]>([]);
  ...
  useEffect(() => {
    const q = query(collection(db, "fbm_batches"), orderBy("createdAt", "desc"));
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const fetchedBatches: FbmBatch[] = [];
      snapshot.forEach((doc) => {
        fetchedBatches.push({ id: doc.id, ...doc.data() as Omit<FbmBatch, 'id'> });
      });
  ...
  const handleCreateBatch = async () => {
    ...
    setIsLoading(true);
    try {
      const docRef = await addDoc(collection(db, "fbm_batches"), {
                ...
                sortedEntries.map((entry) => (
                  <tr key={entry.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <span className="text-xs font-semibold text-gray-600 bg-gray-100 px-2 py-1 rounded-md">{entry.batchName || "N/A"}</span>
                    </td>
