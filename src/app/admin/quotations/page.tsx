...
export default function quotationsPage() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map(
        (doc) => ({ id: doc.id, ...doc.data() } as quotation)
      );
      setquotations(data);
      setIsLoading(false);
    });
  ...
  const filtered = useMemo(() => {
    return quotations.filter((inv) => {
      ...
      // Date range
      if (dateFrom) {
        const invDate = new Date(inv.quotationDate);
  ...
  }, [quotations, searchTerm, dateFrom, dateTo, statusFilter]);

  // Stats (computed from ALL quotations, not filtered)
  const stats = useMemo(() => {
    const total = quotations.length;
    const paid = quotations.filter((i) => i.status === "Paid").length;
    const partial = quotations.filter((i) => i.status === "Partial").length;
    const pending = quotations.filter((i) => i.status === "Pending").length;
    const revenue = quotations.reduce((sum, i) => sum + (i.amountPaid || 0), 0);
      ...
      >
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Create and manage customer quotations • Live from Firebase
          </p>
        </div>
        <Link
      ...
      >
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="relative flex-1 min-w-[250px]">
            ...
            <input
              type="text"
              placeholder="Search by quotation ID, customer name or phone..."
              value={searchTerm}
              onChange={(e) => {
            ...
            <tbody className="divide-y divide-gray-50">
              {isLoading ? (
                ...
                paginatedquotations.map((inv) => {
                  ...
                  const formattedDate = inv.quotationDate
                    ? new Date(inv.quotationDate).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      })
                    ...
                    >
                      ...
                      <td className="px-6 py-5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          ...
                          <button
                            onClick={() =>
                              router.push(`/admin/quotations/${inv.id}/edit`)
                            }
                            className="p-1.5 bg-gray-50 text-gray-500 hover:text-primary hover:bg-gray-200 rounded transition-colors"
