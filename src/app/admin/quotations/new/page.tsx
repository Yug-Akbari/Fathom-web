  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      if (sendMail && customerEmail) {
        ...
        localStorage.setItem(`sendMailType_${docId}`, "Quotation");
      } else if (sendMail && !customerEmail) {
        alert("Quotation saved. Could not schedule email because customer email is missing.");
      }

      router.push(`/admin/quotations/${docId}`);
    } catch (error) {
              ...
              <label className="flex items-center gap-3 cursor-pointer group" htmlFor="showBankDetails">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold text-gray-700 group-hover:text-gray-900 transition-colors">
                    Include Bank Details in Quotation
                  </span>
                </div>
              </label>
              <p className="text-[10px] text-gray-400 mt-1.5 ml-8">
                When checked, bank account details will be printed on the quotation for bank transfer payments.
              </p>
            </div>
          </div>
        </div>
